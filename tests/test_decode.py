...
def test_decodes_a_glyph_cell():
    plate = one(Cell(Form.RING, variant=2, scale=4, ground=0))
    assert decode(render.render(plate)) == plate
...
def test_more_than_two_colours_in_a_cell_is_rejected():
    image = render.render(one(Cell(Form.DISC, variant=6, scale=3, ground=0)))
...
def test_an_undefined_form_variant_pair_is_rejected_at_load():
    ...
    image = render.render(one(Cell(Form.DISC, variant=3, scale=3, ground=0)))
...
def test_load_errors_name_the_cell():
    plate = Plate(2, 1, (
        (Cell(Form.VOID, ground=0), Cell(Form.DISC, variant=6, scale=3, ground=0)),
    ))
