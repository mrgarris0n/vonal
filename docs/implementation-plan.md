...
> **Corrected during execution.** This `_token` writes every non-void channel as a digit, producing output like `o057  +007  ...0` — which round-trips semantically but resembles no hand-written source, so `compile` followed by `disassemble` was not byte-identical. Task 11 surfaced it. The shipped version extracts a single `_channel` helper and applies it to variant, scale **and** ground in both branches: a dot means the channel is zero, uniformly. The `test_emit_writes_the_header_and_aligned_tokens` assertion below therefore reads `"o.51  ...."`, not `"o051  ...0"`. See spec §5, "Canonical form".
...
#
# Row 0 seeds the counter and drops the eye into the loop. The turn at (1,0)
# sets the heading south, so the cell it lands on -- (1,1) -- must turn it east
# again before the body runs: headings persist until a triangle changes them.
#
# Row 1 is the loop body and is exactly as wide as the plate, so running off the
...
o.57  v0.7  ....  ....  ....  ....  ....
v2.7  >0.7  %0.7  +0.7  o.17  #1.7  %0.7
....  ....  ....  ....  ....  ....  ....
```

> **Corrected during execution.** This plate was originally 6x3 and omitted the
> `>0.7` turn-east at `(1,1)`, and the trace table below originally showed the eye
> moving east from `(1,1)`. That was fiction: the turn at `(1,0)` sets the heading
> **south**, headings persist until a triangle changes them, and nothing turned it
> back — so the eye fell straight into the void and halted in 4 steps with no
> output. Task 12's implementer caught it by reporting the divergence rather than
> tuning the plate to match. The version above is verified by execution, not by
> hand: it prints `54321` and halts in 38 steps.
...
| `(0,0)` | push 5 | `[5]` |
| `(1,0)` | turn south | `[5]` |
| `(1,1)` | turn east — the heading is south on arrival | `[5]` |
| `(2,1)` | dup | `[5, 5]` |
| `(3,1)` | out num → `5` | `[5]` |
| `(4,1)` | push 1 | `[5, 1]` |
| `(5,1)` | sub | `[4]` |
