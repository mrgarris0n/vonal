...
- Produces: `vonal.notation.emit(plate: Plate) -> str`, and `tests.strategies.plates()` — a Hypothesis strategy generating **valid** `Plate`s, reused by Task 7.
...
def test_decodes_a_glyph_cell():
    plate = one(Cell(Form.RING, variant=5, scale=4, ground=0))
    assert decode(render.render(plate)) == plate
